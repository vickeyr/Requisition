...
import { ApiHelper3, sessionKeys } from '../../utilities/ConstantVariable';
import { InputTextarea } from 'primereact/inputtextarea';
import { SelectButton } from 'primereact/selectbutton';
import { FileUpload } from 'primereact/fileupload';

class MobileSuppliersScreen extends React.Component {
    ...
    constructor(props) {
        ...
        try {
            ...
            if (undefined !== search && null !== search && (search + "").length > 0) {
                let updatedSearch = (search + "").substring(1, (search + "").length);
                if (undefined !== updatedSearch && null !== updatedSearch && updatedSearch.length > 0) {
                    let keyValuePairs = updatedSearch.split("&");
                    if (undefined !== keyValuePairs && null !== undefined !== keyValuePairs && keyValuePairs.length > 0) {
                        if (this.checkKeyWithItsValue(keyValuePairs[0], "authToken")) {
                            if (this.checkKeyWithItsValue(keyValuePairs[1], "loginUserId")) {
                                if (this.checkKeyWithItsValue(keyValuePairs[2], "companyId")) {
                                    let authToken = keyValuePairs[0].split("n=")[1];
                                    if (undefined !== authToken && null !== authToken && authToken.length > 0) {
                                        let lgnUsrId = keyValuePairs[1].split("=")[1];
                                        if (undefined !== lgnUsrId && null !== lgnUsrId && lgnUsrId.length > 0) {
                                            sessionStorage.setItem(sessionKeys.authToken, authToken);
                                            sessionStorage.setItem(sessionKeys.loginUserId, lgnUsrId);
                                            this.setState({
                                                authToken: authToken,
                                                loginUserId: lgnUsrId
                                            }, setTimeout(() => { this.getCompanyById(keyValuePairs[2].split("=")[1], authToken, lgnUsrId) }, 500));
                                        } else {
    ...
    checkKeyWithItsValue(keyValuePairObj, key) {
        if (keyValuePairObj.startsWith(key)) {
            let splittedKeyValue = keyValuePairObj.split("=");
            if (undefined !== splittedKeyValue && null !== splittedKeyValue && splittedKeyValue.length >= 2) {
                if ((splittedKeyValue[0] + "") === key && (splittedKeyValue[1] + "").length > 0) {
                    return true;
                } else {
                    return false;
                }
    ...
    getCompanyById = async (companyId, authToken, lgnUsrId) => {
        let reqObj = {
            extraVariable: companyId
        };
        ApiHelper3("getcompanybyid", reqObj, "POST", false, false, false, lgnUsrId, authToken).then(data => {
            if (undefined !== data && null !== data && undefined !== data.respObject && null !== data.respObject) {
                this.setState({ companyObj: data.respObject, activeTabIndex: 0 });
                setTimeout(() => {
                    this.loadData(this.state.activeTabIndex, data.respObject, authToken, lgnUsrId);
                }, 500);
            } else {
    ...
    onTabChange(index) {
        ...
        this.loadData(index, this.state.companyObj, sessionStorage.getItem(sessionKeys.authToken), sessionStorage.getItem(sessionKeys.loginUserId));
    }

    loadData = async (index, companyObj, authToken, lgnUsrId) => {
        let reqObj = {
            extraVariable: (index === 0) ? "Active," + companyObj?.id : "InActive," + companyObj?.id
        };
        ApiHelper3("getallsuppliersbyprojectid", reqObj, "POST", false, false, false, lgnUsrId, authToken).then(data => {
    ...
    onAdd = () => {
        this.setState({ dataObj: this.state.emptyDataObj, isEdit: false, showLoader: "flex" });
        this.loadDependenciesData(false, this.state.emptyDataObj);
    }

    loadDependenciesData = async (isEdit, dataObj) => {
        let reqObj = {
            extraVariable: "Active,forRequisition"
        };
        ApiHelper3("getalltransfertype", reqObj, "POST", false, false, false, sessionStorage.getItem(sessionKeys.loginUserId), sessionStorage.getItem(sessionKeys.authToken)).then(data => {
            if (undefined !== data && null !== data && undefined !== data.respList && null !== data.respList) {
                                ...
                                if (re.test(this.state.dataObj.emailId)) {
                                    ...
                                    if (this.validateFeild(this.state.dataObj.markAsCommonSupplier + "")) {
                                        if (this.validateFeild(this.state.dataObj.address)) {
                                            ...
                                            this.toast.show({ severity: 'error', summary: 'Enter Address', detail: '' });
                                        }
                                    } else {
    ...
    saveOrUpdateObject = () => {
        ...
        ApiHelper3(this.state.isEdit ? "updatesupplier" : "createsupplier", reqObj, "POST", false, false, false, sessionStorage.getItem(sessionKeys.loginUserId), sessionStorage.getItem(sessionKeys.authToken)).then(data => {
            if (undefined !== data && null !== data) {
                switch (data.statusCode) {
                    case 0:
                        this.setState({ dataObj: this.state.emptyDataObj, showAddForm: false });
                        this.loadData(this.state.activeTabIndex, this.state.companyObj, sessionStorage.getItem(sessionKeys.authToken), sessionStorage.getItem(sessionKeys.loginUserId));
    ...
    deleteObject = () => {
        ...
        ApiHelper3("deletesupplier", reqObj, "POST", false, false, false, sessionStorage.getItem(sessionKeys.loginUserId), sessionStorage.getItem(sessionKeys.authToken)).then(data => {
            if (undefined !== data && null !== data) {
                switch (data.statusCode) {
                    case 0:
    ...
    rowExpansionTemplate(data) {
        return (
            <div className="orders-subtable">
                {/* <center> */}
                <h5>Payment Details for {data.supplierName}</h5>
            ...
            <div className="p-grid crud-demo">
                <div className="p-col-12">
                    <div className="card">
                        ...
                        <Dialog visible={this.state.showAddForm} style={{ width: '95%' }} header={(this.state.isEdit ? "Edit " : "Add ") + "Supplier"} modal className="p-fluid" footer={this.objectDialogFooter} onHide={this.hideDialog}>
                        ...
                        <Dialog visible={this.state.showDeleteForm} style={{ width: '95%' }} header="Confirm" modal footer={this.deleteDialogFooter} onHide={this.hideDialog}>
...
export default withRouter(MobileSuppliersScreen);